      ...
      <div class="py-8 sm:py-20 border-b border-gray-300">
        <header class="text-center mb-8">
          ...
          <p class="text-gray-700 leading-normal text-sm sm:text-base">
            <span v-if="post.author">von <g-link :to="`${post.author.path}/`" class="text-gray-700 capitalize border-b border-transparent hover:border-gray-400 transition-colors duration-300" v-if="post.author">{{ titleCase(post.author.title) }}</g-link></span>
            ...
            <span>{{ post.timeToRead }} Min. Lesezeit</span>
          </p>
        </header>
...
export default {
  ...
  computed: {
    formattedPublishDate() {
      return moment(this.post.datetime).format('DD.MM.YYYY');
    },
  },
  methods: {
